...
enum InnerPayloadType {
  ...
  /// One slice of a chunked "video circle" (front-camera short clip).
  /// Identical to [AudioChunk] on the wire — see [VideoChunk].
  videoChunk(0x40);
...
/// One slice of a chunked "video circle". Same wire layout as [AudioChunk]
/// — we share the codec shape so reassembly can reuse the audio path's
/// pacing logic. Mime is e.g. `video/mp4`.
class VideoChunk {
  VideoChunk({
    ...
    required this.mime,
    required this.data,
  })  : assert(videoId.length == idLen, 'videoId must be $idLen B'),
        assert(seq >= 0 && seq < 0x10000, 'seq out of u16 range'),
        assert(total >= 1 && total < 0x10000, 'total out of u16 range'),
        assert(seq < total, 'seq must be < total'),
        assert(durationMs >= 0 && durationMs <= 0xFFFFFFFF,
  ...
  Uint8List encode() {
    ...
    var c = 0;
    out.setRange(c, c += idLen, videoId);
    out[c++] = (seq >> 8) & 0xff;
    out[c++] = seq & 0xff;
    out[c++] = (total >> 8) & 0xff;
    out[c++] = total & 0xff;
  ...
  static VideoChunk decode(Uint8List bytes) {
    if (bytes.length < idLen + 2 + 2 + 4 + 1 + 2) {
      throw const FormatException('video chunk truncated');
    }
    var c = 0;
    final id = Uint8List.fromList(bytes.sublist(c, c += idLen));
    final seq = (bytes[c] << 8) | bytes[c + 1];
    ...
    final mimeLen = bytes[c++];
    if (bytes.length < c + mimeLen + 2) {
      throw const FormatException('video chunk mime overrun');
    }
    final mime = utf8.decode(
      bytes.sublist(c, c + mimeLen),
      allowMalformed: true,
    ...
    c += 2;
    if (bytes.length < c + dataLen) {
      throw const FormatException('video chunk data overrun');
    }
    final data = Uint8List.fromList(bytes.sublist(c, c + dataLen));
    return VideoChunk(
      videoId: id,
