...
class _CodePad extends StatefulWidget {
  const _CodePad({required this.title, required this.hint});
...
class _CodePadState extends State<_CodePad> {
  ...
  void _press(String digit) {
    if (_code.length >= _CodePad.maxLength) return;
  ...
  void _submit() {
    if (_code.length < _CodePad.minLength) return;
  ...
  Widget build(BuildContext context) {
    final t = AppLocalizations.of(context);
    final ready = _code.length >= _CodePad.minLength;
          ...
          children: [
            ...
            SizedBox(
              ...
              child: Row(
                ...
                children: [
                  for (var i = 0;
                      i < (_code.length > _CodePad.minLength
                          ? _code.length
                          : _CodePad.minLength);
                      i++)
                    Padding(
