...
class MapPresenceController extends Notifier<int> {
  ...
  int build() {
    ...
    ref.listen<Map<String, KnownPeer>>(knownPeersControllerProvider,
        (before, after) {
      // Blocking a map friend takes the pin back from them at once, rather
      // than only leaving them out of the next beacon: their last copy would
      // otherwise stand on their map for the rest of its six minutes. App Store
      // review asked, under guideline 5.1.2(i), for a way to block other users
      // of a location map, and a block has to reach the map to be one.
      final friends = ref.read(mapFriendsControllerProvider);
      final newlyBlocked = [
      ...
      if (newlyBlocked.isNotEmpty) {
        DebugLog.instance.log(
          'MAP',
          'blocked ${newlyBlocked.length} map friend(s) — withdrawing the pin '
              'from them',
        );
  ...
  Future<void> _retract(List<String> peers) async {
  ...
  /// Map friends a beacon may go to: every one not blocked.
  ///
  /// A blocked contact stays in the friend list, so unblocking restores the
  /// pairing without a new invite, but receives nothing while blocked.
