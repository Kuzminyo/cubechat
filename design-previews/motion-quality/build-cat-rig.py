...
# The furry side of a forepaw is used for holding and face-touching gestures.
# The existing pad-facing drawing remains for rigid waving/open-palm gestures.
paw_source=rig.key(Image.open(ROOT/'cat-paw-fur-source.png').convert('RGB'))
fur_tip=paw_source.crop((0,0,paw_source.width,round(paw_source.height*.61)))
fur_tip=ImageOps.contain(fur_tip,(53,55),Image.Resampling.LANCZOS)
pa=np.array(fur_tip.getchannel('A')).astype(float)
pa*=np.clip((fur_tip.height-1-np.arange(fur_tip.height))[:,None]/9,0,1)
fur_tip.putalpha(Image.fromarray(np.uint8(pa)))
fur_texture=paw_source.crop((round(paw_source.width*.22),round(paw_source.height*.66),round(paw_source.width*.79),round(paw_source.height*.88))).convert('RGB').resize((512,512),Image.Resampling.BILINEAR)

def smooth_arm(im,shoulder,target,side):
 # One continuous foreleg, from the actual shoulder to the paw. Previously only
 # the lower segment was visible, leaving a disconnected capsule on the chest.
 origin=np.array(shoulder,float);goal=np.array(target,float);axis=goal-origin
 distance=np.linalg.norm(axis);u=axis/max(distance,.01)
 # Bend outward in screen space, independently of whether the paw is raised.
 midpoint=(origin+goal)/2
 bend=np.array([side*min(25.,distance*.3),max(0.,32.-distance*.2)])
 control=midpoint+bend
 pts=[]
 for t in np.linspace(0,1,60):
  point=(1-t)**2*origin+2*(1-t)*t*control+t*t*goal
  pts.append(tuple(point))
 def mask(width):
  m=Image.new('L',(1024,1024));d=ImageDraw.Draw(m);line=[(round(x*2),round(y*2)) for x,y in pts]
  d.line(line,fill=255,width=width*2,joint='curve')
  for x,y in [pts[0],pts[-1]]:d.ellipse((x*2-width,y*2-width,x*2+width,y*2+width),fill=255)
  return m.resize((512,512),Image.Resampling.LANCZOS)
 outer=mask(43);inner=mask(38)
 arm=Image.new('RGBA',(512,512),'#5b5c3a');arm.putalpha(outer)
 fill=fur_texture.convert('RGBA');fill.putalpha(inner);arm.alpha_composite(fill)
 # Blend only the shoulder attachment into the existing body fur; the distal
 # outline stays complete. This removes the detached round cap at the elbow.
 yy,xx=np.mgrid[:512,:512];blend=np.clip(np.sqrt((xx-origin[0])**2+(yy-origin[1])**2)/17,0,1)
 arm.putalpha(Image.fromarray(np.uint8(np.array(arm.getchannel('A'))*blend)))
 angle=-side*12
 hand=rig.layer(fur_tip,tuple(goal),(fur_tip.width/2,fur_tip.height*.43),angle)
 return Image.new('RGBA',(512,512)),arm,hand
