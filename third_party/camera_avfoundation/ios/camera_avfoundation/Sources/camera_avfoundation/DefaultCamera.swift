...
final class DefaultCamera: NSObject, Camera {
  ...
  init(configuration: CameraConfiguration) throws {
    ...
    if configuration.mediaSettings.framesPerSecond != nil {
      try configureRequestedFrameRate()
    } else {
  ...
  // Reuse the supported-format selection for a new sensor during persistent
  // recording. Otherwise a 60 fps selfie falls back to the rear sensor's
  // default cadence after switching, even when that sensor supports 60.
  private func configureRequestedFrameRate() throws {
