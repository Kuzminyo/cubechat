...
- (void)getDisplayMedia:(NSDictionary*)constraints result:(FlutterResult)result {
  ...
  // cubechat: the broadcast picker was opened by performing the private
  // `buttonPressed:` selector on RPSystemBroadcastPickerView, which App Review
  // rejected build 1080 for (guideline 2.5.1). cubechat has no screen sharing,
  // so nothing asks for the picker; the call is gone. See CUBECHAT_PATCH.md.
  if (presentBroadcastPicker) {
    NSLog(@"broadcast picker is not available in this build");
  }
#endif
