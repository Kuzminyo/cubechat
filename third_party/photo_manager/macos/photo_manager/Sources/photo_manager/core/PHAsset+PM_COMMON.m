...
- (NSString *)title {
    ...
    // cubechat: upstream read the private `filename` key by KVO first, which
    // App Review rejected build 1080 for (guideline 2.5.1, "PHAsset.filename").
    // The public PHAssetResource path below was already its fallback; it is now
    // the only path. See third_party/photo_manager/CUBECHAT_PATCH.md.
