...
## Что этот план оставил следующим, найденное при исполнении

Записано здесь, а не в рабочем журнале, потому что журнал живёт одну сессию, а
это нужно тому, кто возьмётся за план «Медиа и экран».

- **Потерянное подтверждение оставляет дыру, которую восьмисекундная сдача
  закрывает лишь наполовину.** Ветка `accept` в `handleSignal` срабатывает
  только из фазы `ringing`. Если подтверждение «звоню» потерялось, а принятие
  дошло раньше, чем истекли восемь секунд, звонящий всё ещё в `dialing` и молча
  выбрасывает принятие. Отбой, который теперь уходит по истечении, лечит
  вызываемого, но не этот случай. Реле теряют примерно одно событие из
  пятидесяти пяти — измерено и записано в `messaging_service.dart`.
- **У фазы `connecting` нет дедлайна.** Все остальные нетерминальные фазы
  вооружены таймером, эта выходит только когда снаружи позовут
  `mediaConnected` или `mediaFailed`. В плане медиа это закроет обработчик
  отказа ICE, но машина, которая владеет «всей жизнью звонка», должна владеть и
  этим сроком.
- **`sendCallSignal` возвращает `Future<void>` и выбрасывает число звеньев,
  которое ему отдал внутренний путь отправки.** Спека требует, чтобы
  недоступные реле давали внятный отказ, а не тихую деградацию; сейчас сигнал,
  не дошедший ни до кого, неотличим от доставленного, и машина узнаёт об этом
  через восемь секунд под чужим ярлыком.
- **На проигранном встречном вызове `notifyListeners` срабатывает дважды
  подряд**, так что слушатель увидит мимолётную фазу `ended` перед `incoming`.
  Виджету это ничем не грозит — перестройка откладывается до следующего кадра,
  — но невиджетный слушатель, который на `ended` сносит медиа, снесёт его зря.
- **Принятая на проводе причина отбоя нигде не читается.** `decline` всегда
  записывается как `declined`, `hangup` как `hungUp`, какой бы байт ни пришёл.
  Сегодня это безвредно: история спрашивает только «ответили или нет».
- **Запись о звонке покажется как base64 там, где строку берут без сообщения.**
  `storedTextPreview` отдаёт нестикерный текст как есть, и два экрана зовут
  именно его. Это давнее поведение, общее с `cubechat:loc:v1:`, но дефектом оно
  станет в тот момент, когда записи начнут писаться по-настоящему.
